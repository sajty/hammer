...
export OUTPUT_VARIABLES="$OUTPUT_VARIABLES ANDROID_SDK ANDROID_NDK ANDROID_STANDALONE_TOOLCHAIN CROSS_COMPILER SYSROOT CONFIGURE_CROSS_COMPILE  CMAKE_CROSS_COMPILE"
...
if [ x"$COMPILE_FLAGS" = x"" ] ; then
  pexport CFLAGS="$CFLAGS $COMPILE_FLAGS"
else
  if [ "$DEBUG_BUILD" = "1" ] ; then
    pexport CFLAGS="$CFLAGS -O0 -g -DDEBUG -D_DEBUG"
  else
    pexport CFLAGS="$CFLAGS -O2 -DNDEBUG"
  fi
fi
pexport LDFLAGS="$LDFLAGS -L$PREFIX/lib $LINK_FLAGS"
pexport CONFIGURE_FLAGS="$CONFIGURE_FLAGS --prefix=$PREFIX"
...
if [ "$CROSS_COMPILE" = "1" ] && [[ "$TARGET_OS" = "android" ]] ; then
  ...
  pexport PKG_CONFIG_PATH=$PREFIX/lib/pkgconfig
  ...
  pexport CPATH="$TOOLCHAIN/include/c++/4.8/${CROSS_COMPILER}${THUMB_DIR}"
  pexport CPATH="$CPATH:$TOOLCHAIN/lib/gcc/${CROSS_COMPILER}/4.8/include"
  pexport CPATH="$CPATH:$TOOLCHAIN/include/c++/4.8"
  ...
  pexport LIBRARY_PATH="$TOOLCHAIN/$CROSS_COMPILER/lib${THUMB_DIR}:$TOOLCHAIN/lib/gcc/$CROSS_COMPILER/4.8${THUMB_DIR}:$SYSROOT/usr/lib:$PREFIX/lib"
